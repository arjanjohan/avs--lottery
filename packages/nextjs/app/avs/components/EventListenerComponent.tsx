...
const EventListenerComponent: React.FC = () => {
  ...
  const { data: serviceManagerContract } = useScaffoldContract({
    contractName: "LotteryServiceManager",
  });

  useEffect(() => {
    const fetchStatuses = async () => {
      if (!events || events.length === 0 || !serviceManagerContract || !address) return;

      const statuses = await Promise.all(events.map(async (event) => {
        const isTaskResponded = await serviceManagerContract.read.allTaskResponses([address, event.args.taskIndex]);
    ...
    fetchStatuses();
  }, [events, serviceManagerContract, address]);
